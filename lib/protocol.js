...
// TODO : protocollib names aren't the best around here
  ...
  login: {
    toClient: {
      ...
      set_compression:      {id: 0x03, fields: [
...
function createPacketBuffer(packetId, state, params, isServer, compression) {
  var length = 0;
  ...
  var offset = writeVarInt(packetId, data, 0);
  packet.forEach(function(fieldInfo) {
    var value = params[fieldInfo.name];
    if(typeof value === "undefined") value = 0; // TODO : Why ?
    offset = write(value, data, offset, fieldInfo, params);
  });

  var dataLength = 0;
  if (compression.enabled && data.length > compression.threshold) {
    data = zlib.deflateSync(data);
    dataLength = length = data.length;
    length += sizeOfVarInt(length);
  }
  
  var size = length + sizeOfVarInt(length);
  var buffer = new Buffer(size);
  offset = writeVarInt(length, buffer, 0);
  if (compression.enabled) {
    offset = writeVarInt(dataLength, buffer, offset);
  }
  offset = writeBuffer(data, buffer, offset);
  return buffer;
}

function parsePacket(buffer, state, isServer, packetsToParse, compression) {
  ...
  if (compression.enabled) {
    ...
    var dataLength = dataLengthField.value;
    cursor += dataLengthField.size;

    if (dataLength != 0) {
      buffer = zlib.inflateSync(buffer.slice(cursor));
    }
  }
